...
module.exports = {
  meta: {
    ...
    messages: {
      requireMultiline: "Avoid using variables named '{{ name }}'"
  ...
  create(context) {
    ...
    return {
      ...
      VariableDeclaration(node) {
        ...
        const objExpression = declarator.init;

        if (objExpression.type === 'ObjectExpression') {
          const propCount = objExpression.properties.length;
          const { line: startLine } = objExpression.loc.start;
          const { line: endLine } = objExpression.loc.end;
